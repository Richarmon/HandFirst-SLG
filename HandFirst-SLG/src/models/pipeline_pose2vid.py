...
from ..models.context import get_context_scheduler
...
class MASKPose2VideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Prepare original video latents
        # video_tensor (f, c, h, w)
        # mask_tensor (f, c, h, w) c=1
        video_length = video_tensor.shape[0]

        video_tensor = video_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        video_latents = self.vae.encode(video_tensor).latent_dist.mean
        video_latents = video_latents * 0.18215  # (f, c, h, w)
        ...
        mask_tensor = mask_tensor.repeat(1, video_latents.shape[1], 1, 1)  # (c, f, h, w), c=video_latents.c
        ...
        # first step input replace
        if len(timesteps) - num_final_no_mask_steps > 0:
            print("using mask mechanism")
            latents = video_latents * mask_tensor + latents * (1 - mask_tensor)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                num_context_batches = math.ceil(len(context_queue) / context_batch_size)
                global_context = []
                for i_1 in range(num_context_batches):
                    global_context.append(
                        context_queue[
                            i_1 * context_batch_size : (i_1 + 1) * context_batch_size
                        ]
                    )
                ...
                if i < len(timesteps) - num_final_no_mask_steps:
                    print(f"{i = } < {len(timesteps) - num_final_no_mask_steps = }, using mask, {t = }")
                    latents = video_latents * mask_tensor + latents * (1 - mask_tensor)
                else:
                    print(f"{i = } >= {len(timesteps) - num_final_no_mask_steps = }, not using mask, {t = }")
